...
namespace BarberShop.Feature.Command.User.CreateUser
{
    public class CreateUserCommandHandler : IRequestHandler<CreateUserCommand, ResponseDTO>
    {
        ...
        {
            var result = await _user.RegisterAsync(request.UserName,request.Password,request.FullName,request.Bio,request.ImageUrl,request.StartTime,request.EndTime,request.PhoneNumber,request.Role);
            return result;
